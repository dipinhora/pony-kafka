...
use @LZ4F_createCompressionContext[LZ4FError](
  cctx_p: Pointer[LZ4FCompressionContext], version: U32)
use @LZ4F_compressBegin[USize](cctx: LZ4FCompressionContext,
  buf: Pointer[U8] tag, buf_size: USize, pref_p: MaybePointer[LZ4FPreferences])
use @LZ4F_freeCompressionContext[LZ4FError](cctx: LZ4FCompressionContext)
use @LZ4F_createDecompressionContext[LZ4FError](
  dctx_p: Pointer[LZ4FDecompressionContext], version: U32)
use @LZ4F_getFrameInfo[LZ4FError](dctx: LZ4FDecompressionContext,
  frame_info: MaybePointer[LZ4FFrameInfo], buf: Pointer[U8] tag,
  buf_size: Pointer[USize])
use @LZ4F_freeDecompressionContext[LZ4FError](dctx: LZ4FDecompressionContext)

// TODO: Remove USize from union for buf pointer
use @LZ4F_compressUpdate[USize](cctx: LZ4FCompressionContext,
  buf: (USize | Pointer[U8] tag), buf_size: USize, src_buf: Pointer[U8] tag,
  src_size: USize, copts_p: MaybePointer[LZ4FCompressOptions])
use @LZ4F_compressEnd[USize](cctx: LZ4FCompressionContext,
  buf: (USize | Pointer[U8] tag), buf_size: USize,
  copts_p: MaybePointer[LZ4FCompressOptions])
use @LZ4F_decompress[USize](dctx: LZ4FDecompressionContext,
  buf: (USize | Pointer[U8] tag), buf_size: Pointer[USize],
  src_buf: (USize | Pointer[U8] tag), src_size: Pointer[USize],
  copts_p: MaybePointer[LZ4FDecompressOptions])
  ...
  =>
    ...
    if @LZ4F_isError(err) != 0 then
      logger(Error) and logger.log(Error,
        "LZ4 couldn't create decompression context! LZ4 error message: " +
        String.copy_cstring(@LZ4F_getErrorName(err)))
      error
    end
    ...
    if @LZ4F_isError(err) != 0 then
      logger(Error) and logger.log(Error,
        "LZ4 couldn't read frame info! LZ4 error message: " +
        String.copy_cstring(@LZ4F_getErrorName(err)))
      @LZ4F_freeDecompressionContext(dctx)
      error
    end

    let out_size = if (fi.content_size == 0) or
      (fi.content_size.usize() > (data.size() * 255)) then
        data.size() * 255
      else
    ...
    while data_offset < data.size() do
      ...
      err = @LZ4F_decompress(dctx, buffer.cpointer().usize() + buf_offset,
        addressof buf_size, data.cpointer().usize() + data_offset,
        addressof data_size, dopts_p)
      if @LZ4F_isError(err) != 0 then
        logger(Error) and logger.log(Error,
          "LZ4 couldn't decompression data! LZ4 error message: " +
          String.copy_cstring(@LZ4F_getErrorName(err)))
        @LZ4F_freeDecompressionContext(dctx)
        error
      end
      ...
      if buf_offset > buffer.size() then
        logger(Error) and logger.log(Error, "LZ4 buffer offset is larger " +
          "than buffer size! This should never happen!")
        @LZ4F_freeDecompressionContext(dctx)
        error
      end

      if data_offset > data.size() then
        logger(Error) and logger.log(Error, "LZ4 input data offset is larger " +
          "than input data size! This should never happen!")
        @LZ4F_freeDecompressionContext(dctx)
        error
      end
...
// based on https://github.com/edenhill/librdkafka/blob/master/src/rdkafka_lz4.c
  ...
  =>
    ...
    if @LZ4F_isError(max_len) != 0 then
      logger(Error) and logger.log(Error, "LZ4 couldn't determine output " +
        "size for compression! LZ4 error message: " +
        String.copy_cstring(@LZ4F_getErrorName(max_len)))
      error
    end
    ...
    if @LZ4F_isError(err) != 0 then
      logger(Error) and logger.log(Error,
        "LZ4 couldn't create compression context! LZ4 error message: " +
        String.copy_cstring(@LZ4F_getErrorName(err)))
      error
    end
    ...
    if @LZ4F_isError(err) != 0 then
      logger(Error) and logger.log(Error,
        "LZ4 couldn't begin compression! LZ4 error message: " +
        String.copy_cstring(@LZ4F_getErrorName(err)))
    ...
    for d in data.values() do
      err = @LZ4F_compressUpdate(cctx, buffer.cpointer().usize() +
        bytes_written, buffer.size() - bytes_written, d.cpointer(), d.size(),
        copts_p)
      if @LZ4F_isError(err) != 0 then
        logger(Error) and logger.log(Error,
          "LZ4 compression error! LZ4 error message: " +
          String.copy_cstring(@LZ4F_getErrorName(err)))
        @LZ4F_freeCompressionContext(cctx)
        error
      end
    ...
    err = @LZ4F_compressEnd(cctx, buffer.cpointer().usize() + bytes_written,
      buffer.size() - bytes_written, copts_p)
    if @LZ4F_isError(err) != 0 then
      logger(Error) and logger.log(Error,
        "LZ4 couldn't end compression! LZ4 error message: " +
        String.copy_cstring(@LZ4F_getErrorName(err)))
      @LZ4F_freeCompressionContext(cctx)
      error
    end
