...
class iso _TestError is _StringLoggerTest
  ...
  fun log(logger: Logger[String]) =>
    logger(Error) and logger.log(Error, "error message")
    logger(Warn) and logger.log(Warn, "warn message")
    logger(Info) and logger.log(Info, "info message")
    logger(Fine) and logger.log(Fine, "fine message")
...
class _TestObjectLogging is _LoggerTest[U64]
  ...
  fun log(logger': Logger[U64]) =>
    logger'(Error) and logger'.log(Error, U64(42))
...
trait _StringLoggerTest is _LoggerTest[String]
  fun logger(
    ...
    : Logger[String]
  =>
    StringLogger(level', stream', false, formatter')
